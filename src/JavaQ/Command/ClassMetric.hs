...
classmetricsCmd =
 ...
 where
  fn cn bts =
    ...
    in
      case first show (readClassFile' True bts) >>= convertClass of
        Right cls -> ClassMetric
          ...
          , cmErrorMessage = if cn /= cls ^. className
                               then "Expected other classname: " ++ Text.unpack
                                 (cls ^. className . fullyQualifiedName)
                               else ""
          }
        Left msg -> ClassMetric { cmName         = cn
                                , cmSize         = ln
