...
runCommand classloader (Command _ (fmt :: Format a) tp) = do
 ...
 where
  runCommandType = \case
    Stream gran fn -> do
      ...
      case gran of
        ...
        Classes -> inClasspool $ streamClasses (liftIO . applyFormat fmt . fn')

        Methods -> inClasspool $ do
          streamClasses $ \cls -> liftIO $ do
            forMOf_ (classMethods . folded) cls
              $ \m -> applyFormat fmt (fn' (cls ^. className, m))

        Fields -> inClasspool $ do
          streamClasses $ \cls -> liftIO $ do
            forMOf_ (classFields . folded) cls
              $ \f -> applyFormat fmt (fn' (cls ^. className, f))
  ...
  preludeFormat = \case
    Csv h _ -> liftIO . BL.putStr $ Csv.encode [h]
    _       -> return ()

  applyFormat = \case
    Txt fn    -> Text.putStrLn . fn
    Csv _ fn  -> BL.putStr . Csv.encode . fn
    Json   fn -> BL.putStrLn . Json.encode . fn
    Binary fn -> BL.putStrLn . Binary.encode . fn
    ...
    void . flip runCachedClassPoolT opts $ do
      view cfgClassNames >>= \case
        ...
        classNames' -> do
          ...
          --   True -> do
          --     void . computeClassClosureM classSet $ \(_, clss) -> cplocal $ do
